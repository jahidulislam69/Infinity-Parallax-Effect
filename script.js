const projectData = [
    {title: "Sit To Calm Me", image: "Assets/1.JPG", isAlternate: false},
    {title: "These Vibrant Colour", image: "Assets/2.JPG", isAlternate: true},
    {title: "Sunset Falls On Me", image: "Assets/3.JPG", isAlternate: false},
    {title: "Look At The Camera", image: "Assets/4.JPG", isAlternate: true},
    {title: "Sometimes I want's To Be A Sailor", image: "Assets/5.JPG", isAlternate: false},
    {title: "Sea Makes Me Happy", image: "Assets/6.JPG", isAlternate: true}
]
...
const update = (scroll) => {
    if (!bounds) return;
    const relatveScroll = -scroll - bounds.top;
    targetTranslateY = relatveScroll * 0.2;
...
const animate = () => {
    ...
    state.projects.forEach((project, index) => {
        ...
        const parallaxImages = state.parallaxImages.get(index);
        if (parallaxImages) {
            parallaxImages.update(state.currentY)
        }
    })
...
const handleWheel = (e) => {
    ...
    const scrollDelta = e.deltaY * config.SCROL_SPEED;
    state.targetY -= Math.max(
        Math.min(scrollDelta, config.MAX_VALOCITY),
        -config.MAX_VALOCITY
    );
};
...
const handleResize = () => {
    ...
    state.projects.forEach((project, index) => {
        project.style.transform = `tramslateY(${index * state.projectHeight}px)`;
